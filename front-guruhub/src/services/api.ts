...
const getApiUrl = () => {
  ...
  if (typeof window !== "undefined") {
    ...
    // Local dev (running directly via localhost/127.0.0.1 without reverse proxy)
    if (host === "localhost" || host === "127.0.0.1") {
      return `${window.location.protocol}//${host}:3000`;
    }

    // Default reverse proxy: use same origin /api (e.g. https://guruhub.cbt-smpht5.my.id/api)
    // This eliminates cross-origin CORS preflight issues entirely!
...
async function request(path: string, options: RequestInit = {}): Promise<any> {
  ...
  // Proactive token refresh if access token is missing in memory
  if (!accessToken && store.refreshToken && !path.startsWith("/auth/")) {
    try {
      if (!refreshPromise) {
  ...
  // Handle blob or standard JSON response
  if ((options as any)?.returnBlob) {
...
export const api = {
  get: <T = any>(path: string, options?: RequestInit): Promise<T> => request(path, { ...options, method: "GET" }),
  ...
  delete: <T = any>(path: string, options?: RequestInit): Promise<T> => request(path, { ...options, method: "DELETE" }),
  download: async (path: string, filename: string) => {
    const blob: Blob = await request(path, { method: "GET", returnBlob: true } as any);
